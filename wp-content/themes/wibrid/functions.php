...
if(!function_exists('qode_google_fonts_styles')) {
	...
	function qode_google_fonts_styles() {
		...
		if(count($fonts_array) > 0) {
			...
			printf("<link href='/css?family=".$default_font_string."|%s&subset=latin,latin-ext' rel='stylesheet' type='text/css'>\r\n", str_replace(' ', '+', $google_fonts_string));
		} else {
			//include default google font that theme is using
			printf("<link href='/css?family=".$default_font_string."' rel='stylesheet' type='text/css'>\r\n");
		}

		if(isset($qode_toolbar)){
			printf("<link href='/css?family=Raleway:400,600' rel='stylesheet' type='text/css'>\r\n");
		}
	}
...
//wp_enqueue_script("carouFredSel", QODE_ROOT."/js/jquery.carouFredSel-6.2.1.min.js",array(),false,true);移到下面了
...
wp_enqueue_script("carouFredSel", QODE_ROOT."/js/jquery.carouFredSel-6.2.1.min.js",array(),false,true);
        ...
        if((isset($qode_options_proya['enable_google_map']) && $qode_options_proya['enable_google_map'] == "yes") || qode_is_ajax_enabled() || qode_has_google_map_shortcode()) :
            wp_enqueue_script("google_map_api", "/js?sensor=false",array(),false,true);
        endif;
