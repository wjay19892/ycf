...
if($verify_result) {//验证成功
    ...
    if($_POST['trade_status'] == 'TRADE_FINISHED' ||$_POST['trade_status'] == 'TRADE_SUCCESS') {    //交易成功结束
        ...
        if($money_info)
        {
        	...
        	{
        		addUserMoney($money_info->ice_user_id, $total_fee);
        	}
        	mysql_query("UPDATE $wpdb->icemoney SET ice_money = '$total_fee', ice_alipay = '$_POST[buyer_email]',ice_success=1, ice_success_time = '".time()."' WHERE ice_num = '$_POST[out_trade_no]'");
        }
        ///////////////////////////////////////////////////
