...
$partner = get_option(ice_ali_partner); 

//安全检验码，以数字和字母组成的32位字符
$key = get_option(ice_ali_security_code); 

//签约支付宝账号或卖家支付宝帐户
$seller_email= get_option(ice_ali_seller_email); 

//交易过程中服务器通知的页面 要用 http://格式的完整路径，不允许加?id=123这类自定义参数
$notify_url		= get_option(siteurl).'/wp-content/plugins/ciphpdown/notify_url.php';

//付完款后跳转的页面 要用 http://格式的完整路径，不允许加?id=123这类自定义参数
//return_url的域名不能写成http://localhost/js_php_utf8/return_url.php ，否则会导致return_url执行无效
$return_url		= get_option(siteurl).'/wp-content/plugins/ciphpdown/alipay_return.php'; 

//网站商品的展示地址，不允许加?id=123这类自定义参数
$show_url		= get_option(siteurl); 

//收款方名称，如：公司名称、网站名称、收款人姓名等
$mainname		= get_option(ice_ali_seller_name); 
